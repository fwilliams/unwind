...
static constexpr float SelectionRadius = 0.15f;

static std::vector<Eigen::Vector2f> create_default_points(int nPoints, float radius) {
  constexpr float M_TAU = 2.f * M_PI;
...
static constexpr const char* PlaneVertexShader = R"(
#version 150
...
static constexpr const char* PlaneFragmentShader = R"(
#version 150
  in vec3 uv;
...
static constexpr const char* PolygonVertexShader = R"(
#version 150
in vec2 position;
...
static constexpr const char* PolygonFragmentShader = R"(
#version 150
...
bool Bounding_Polygon_Widget::mouse_move(int mouse_x, int mouse_y) {
  ...
  float normalized_mouse_x = ((mouse_x / static_cast<float>(width)) - 0.5f) * 2.f;
  float normalized_mouse_y = -(((mouse_y / static_cast<float>(height)) - 0.5f) * 2.f);

  if (left_mouse && is_currently_on_slice && current_edit_element) {
      // map to rendering window
      float mapped_mouse_x = normalized_mouse_x / (500.f / static_cast<float>(width));
      float mapped_mouse_y = normalized_mouse_y / (500.f / static_cast<float>(height));
...
bool Bounding_Polygon_Widget::mouse_down(int button, int modifier) {
    ...
    float normalized_mouse_x = ((viewer->current_mouse_x / static_cast<float>(width)) - 0.5f) * 2.f;
    float normalized_mouse_y = -(((viewer->current_mouse_y / static_cast<float>(height)) - 0.5f) * 2.f);
    ...
    if (left_mouse && is_currently_on_slice) {
        ...
        for (Eigen::Vector2f& p : state.bounding_polygon.polygon_slices[current_slice_id].polygon) {
            float d = sqrt((p.x() - mapped_mouse_x) * (p.x() - mapped_mouse_x) + (p.y() - mapped_mouse_y) * (p.y() - mapped_mouse_y));
...
bool Bounding_Polygon_Widget::post_draw(const Eigen::MatrixXd& PV, int current_vertex_id) {
  ...
  glUniform2f(plane.window_size_location, static_cast<float>(width), static_cast<float>(height));
  std::array<float, 3> dim = state.volume_rendering.parameters.volume_dimensions_rcp;

  std::array<float, 3> ll = { PV.cast<float>().row(2).x(), PV.cast<float>().row(2).y(), PV.cast<float>().row(2).z() };
  std::array<float, 3> lr = { PV.cast<float>().row(3).x(), PV.cast<float>().row(3).y(), PV.cast<float>().row(3).z() };
  std::array<float, 3> ul = { PV.cast<float>().row(1).x(), PV.cast<float>().row(1).y(), PV.cast<float>().row(1).z() };
  std::array<float, 3> ur = { PV.cast<float>().row(0).x(), PV.cast<float>().row(0).y(), PV.cast<float>().row(0).z() };
  ...
  if (is_currently_on_slice) {
    ...
    bool left_mouse = glfwGetMouseButton(viewer->window, GLFW_MOUSE_BUTTON_1) == GLFW_PRESS;
    float mouse_x = ((viewer->current_mouse_x / static_cast<float>(width)) - 0.5f) * 2.f;
    float mouse_y = -(((viewer->current_mouse_y / static_cast<float>(height)) - 0.5f) * 2.f);

    // map to rendering window
    float mapped_mouse_x = mouse_x / (500.f / static_cast<float>(width));
    float mapped_mouse_y = mouse_y / (500.f / static_cast<float>(height));
  ...
  auto it = std::find_if(state.bounding_polygon.polygon_slices.begin(),
                         ...
                         [id = current_vertex_id](const State::BoundingPolygon::Slice& slice) { return slice.vertex_id == id; }
  );
  ...
  ImGui::SameLine();
  if (ImGui::SliderInt("#sliceid", &current_slice_id, 0, state.bounding_polygon.polygon_slices.size() - 1)) {
    current_vertex_id = state.bounding_polygon.polygon_slices[current_slice_id].vertex_id;
  }
  ImGui::SameLine();
  if (ImGui::Button("Next >")) {
    current_slice_id = std::min(current_slice_id + 1, int(state.bounding_polygon.polygon_slices.size() - 1));
    current_vertex_id = state.bounding_polygon.polygon_slices[current_slice_id].vertex_id;
  }
  glPopDebugGroup();
  return false;
