...
void tetmesh_from_triangulation_3(
    const CGAL::Triangulation_3<GT, Tds_, Lds_>& tr,
    Eigen::MatrixXd& TV, Eigen::MatrixXi& TF, Eigen::MatrixXi& TT)
{
       ...
       vit != tr.finite_vertices_end(); ++vit)
  {
    ...
    TV.row(v_idx) = Eigen::RowVector3d(CGAL::to_double(p.x()),
                                       CGAL::to_double(p.y()),
                                       CGAL::to_double(p.z()));
    v_idx += 1;
  }
  ...
  int tri_idx = 0;
  for (Facet_iterator fit = tr.finite_facets_begin();
...
void tetmesh_from_c3t3(
    const CGAL::Mesh_complex_3_in_triangulation_3<Tr>& c3t3,
    Eigen::MatrixXd& TV, Eigen::MatrixXi& TF, Eigen::MatrixXi& TT) {
  using namespace std;
