...
int main(int argc, char * argv[])
{
	...
	app.add_option("input,-i,--input", args.input, "Input image.")->required()->check(CLI::ExistingFile);
	...
	if (args.radius > 0)
	{
		std::cout << "-- Performing offset by radius r = " << args.radius << std::endl;
		if (args.erode)
		{
	...
	/*dexels.toImage([&img] (int x, int y, bool b)
	{
		img.at(x, y) = (b ? BLACK : WHITE);
	});*/
	...
	if (std::ifstream(args.output))
	{
		...
		{
			std::cerr << "-- Output file already exists. Please use -f to force overwriting." << std::endl;
		}
	}
	else
	{
